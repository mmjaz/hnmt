...
class HierarchicalContext(nn.Module):
    ...
    def _get_sent_context(self, query, context_word, index, attn_word):
        ...
        if key_word is not None:
            key_sent = key_word.view(b_size, c_size, t_size, d_size).transpose(1, 2).contiguous().view(b_size * t_size,
                                                                                                       c_size, d_size)
        else:
    ...
    def forward(self, input, query, context, doc_index, batch_i=None):
        ...
        query_word_norm, context_word, key_word, context_word_mask = self._get_word_context(query_word_norm, context,
                                                                                            key, index, in_pad_mask)

        """   Word-level attention   """
        context_word, attn_word = self.word_attn(key_word, context_word,
                                                 query_word_norm, mask=context_word_mask, return_key=key is not None,
                                                 all_attn=True)
        context_sent = self.layer_norm_word(context_word)

        """   Reshape the tensors for applying sentence-level attention   """
        query_sent_norm, context_sent, key_sent, context_sent_mask, attn_word = self._get_sent_context(query_sent_norm,
                                                                                                       context_sent,
                                                                                                       None if key is None else key_word,
                                                                                                       index, attn_word)
